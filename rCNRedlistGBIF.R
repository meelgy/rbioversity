...
#读取RGBIF并选取特定列
bioChina <- readr::read_csv("E:\\GBIFGrid\\Merge.csv")
pbioChina <- bioChina %>%
  select(species,decimalLatitude,decimalLongitude,elevation,individualCount,organismQuantity,gbifID,stateProvince,locality,basisOfRecord,year,month,day)
#乱码write_excel_csv(pbioChina,"E:\\GBIFGrid\\GBIFpro\\pbioChina.csv")
#不乱码
write_csv(pbioChina,"E:\\GBIFGrid\\GBIFpro\\pbioChina_1.csv")

#读取筛选过列的数据
pbioChina <- readr::read_csv("E:\\GBIFGrid\\GBIFpro\\pbioChina_1.csv")



#兽类：分别提取CSIS中存在和不存在的beast
#不存在的beast，在GBIF中匹配
...
#输出主要数据列
write.csv(GBIF_redlist_usCSISBird,"E:\\GBIFGrid\\GBIFpro\\GBIF_redlist_usCSISBird1.csv",fileEncoding = "UTF-8")
